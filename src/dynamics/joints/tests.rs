...
fn revolute_motor_spins_body() {
    ...
    #[cfg(feature = "2d")]
    {
        assert!(angular_velocity.0.abs() > 1.0, "Angular velocity should be significant");
...
fn prismatic_motor_moves_body() {
    ...
    assert!(displacement > 0.5, "Body should have moved significantly: {}", displacement);
}
...
fn revolute_motor_respects_max_torque() {
    ...
    #[cfg(feature = "2d")]
    {
        assert!(angular_velocity.0.abs() < 5.0, "Velocity should be limited by max torque");
    }
    #[cfg(feature = "3d")]
    {
        let speed = angular_velocity.0.length();
...
fn revolute_motor_position_target() {
    ...
    #[cfg(feature = "2d")]
    {
        ...
        assert!(angle.abs() > 0.3, "Motor should have rotated the body: {}", angle);
    }
    #[cfg(feature = "3d")]
    {
        ...
        assert!(signed_angle.abs() > 0.3, "Motor should have rotated the body: {}", signed_angle);
    }
}
...
fn prismatic_motor_position_target() {
    ...
    assert!(displacement.abs() > 0.1 || final_pos.x.abs() > 0.1,
            "Body should have moved: displacement={}, final_x={}", displacement, final_pos.x);
}
